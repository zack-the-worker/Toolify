...
const input = ref('SELECT id, name FROM users WHERE active = 1 ORDER BY name')

const result = computed(() => {
  try {
    return { value: formatSql(input.value), error: '' }
  } catch (e) {
...
<template>
  <div class="tool">
    ...
    <div class="body">
      <textarea v-model="input" class="pane" placeholder="Paste SQL here" spellcheck="false"></textarea>
