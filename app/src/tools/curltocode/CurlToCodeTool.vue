...
const result = computed(() => {
  try {
    ...
    const code = language.value === 'javascript' ? toJavaScriptFetch(parsed) : toPythonRequests(parsed)
...
<template>
  <div class="tool">
    <div class="toolbar">
      <div class="seg">
        <button :class="{ active: language === 'javascript' }" @click="language = 'javascript'">JavaScript (fetch)</button>
        <button :class="{ active: language === 'python' }" @click="language = 'python'">Python (requests)</button>
      </div>
    ...
    <div class="body">
      <textarea v-model="input" class="pane" placeholder="Paste a curl command" spellcheck="false"></textarea>
