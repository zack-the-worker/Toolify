...
const result = computed(() => {
  try {
    return { value: jsonToTypeScript(input.value, rootName.value || 'Root'), error: '' }
  } catch (e) {
...
<template>
  <div class="tool">
    <div class="toolbar">
      <label class="chk">
        Root name
        <input v-model="rootName" class="name-input" spellcheck="false" />
      </label>
    ...
    <div class="body">
      <textarea v-model="input" class="pane" placeholder="Paste JSON here" spellcheck="false"></textarea>
      <textarea :value="result.value" class="pane" placeholder="TypeScript interfaces" spellcheck="false" readonly></textarea>
    </div>
  </div>
</template>
...
.name-input {
  width: 120px; padding: 5px 8px; border-radius: 6px; border: 1px solid var(--border, #444);
  background: var(--input-bg, #1e1e1e); color: var(--text, #ddd); font-family: 'SF Mono', Menlo, Consolas, monospace; font-size: 12.5px;
}
