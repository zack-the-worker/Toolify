...
const mode = ref<'serialize' | 'unserialize'>('unserialize')
const input = ref('a:2:{s:4:"name";s:5:"Alice";s:3:"age";i:30;}')

const result = computed(() => {
  try {
    if (mode.value === 'serialize') {
      return { value: phpSerialize(JSON.parse(input.value)), error: '' }
    }
    return { value: JSON.stringify(phpUnserialize(input.value), null, 2), error: '' }
  } catch (e) {
...
<template>
  <div class="tool">
    <div class="toolbar">
      <div class="seg">
        <button :class="{ active: mode === 'unserialize' }" @click="mode = 'unserialize'">Unserialize → JSON</button>
        <button :class="{ active: mode === 'serialize' }" @click="mode = 'serialize'">JSON → Serialize</button>
      </div>
